...
# xctool-log2junit, gcovr and oclint installed
...
if [[ "$workspaceFile" != "" ]] ; then
	xcodebuildCmdPrefix="xctool-log2junit -workspace $workspaceFile -sdk iphonesimulator ARCHS=i386 VALID_ARCHS=i386 CURRENT_ARCH=i386 ONLY_ACTIVE_ARCH=NO"
else
	xcodebuildCmdPrefix="xctool-log2junit -project $projectFile -sdk iphonesimulator ARCHS=i386 VALID_ARCHS=i386 CURRENT_ARCH=i386 ONLY_ACTIVE_ARCH=NO"
fi	
...
runCommand /dev/stdout $xcodebuildCmdPrefix -scheme "$appScheme" -reporter json-compilation-database:compile_commands.json build
...
if [ "$testScheme" = "" ]; then
	...
	runCommand sonar-reports/TEST-report.xml $xcodebuildCmdPrefix -scheme "$testScheme" GCC_GENERATE_TEST_COVERAGE_FILES=YES GCC_INSTRUMENT_PROGRAM_FLOW_ARCS=YES test > xcodebuidl.log
